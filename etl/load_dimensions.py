...
CREATE TABLE IF NOT EXISTS dim_collector (
    collector_sk     INTEGER PRIMARY KEY AUTOINCREMENT,
...
# dim_branch
# ---------------------------------------------------------------------------
def load_dim_branch(conn: sqlite3.Connection):
    ...
    conn.executemany(
        "INSERT OR IGNORE INTO dim_branch (branch_code_clean, branch_name, city, region) VALUES (?,?,?,?)",
        rows
    )
    conn.commit()
...
# dim_customer
# ---------------------------------------------------------------------------
def load_dim_customer(conn: sqlite3.Connection):
...
def load_dim_product(conn: sqlite3.Connection):
    rows = []
    # CoreBank
    for code, name in CB_PRODUCT_NAMES.items():
        rows.append(("COREBANK", code, name, None))

    # CoreCard: distinct (card_type, product_type) combos
    ...
    conn.executemany(
        """INSERT OR IGNORE INTO dim_product
           (product_source, product_code, product_name, card_type)
           VALUES (?,?,?,?)""",
        rows
    )
    conn.commit()
...
# dim_collector
# ---------------------------------------------------------------------------
def load_dim_collector(conn: sqlite3.Connection):
    df = pd.read_csv(os.path.join(DATA_DIR, "collector_staff.csv"))

    # Build branch_code_clean → branch_sk lookup
    ...
    for _, r in df.iterrows():
        ...
        branch_sk = branch_map.get(clean, 1)  # default to first branch if unknown
        rows.append((
    ...
    conn.executemany(
        """INSERT OR IGNORE INTO dim_collector
...
# dim_date
# ---------------------------------------------------------------------------
def load_dim_date(conn: sqlite3.Connection):
    start = date(2020, 1, 1)
    end   = date(2034, 12, 31)
    rows  = []
    current = start
...
# scoring_config seed
# ---------------------------------------------------------------------------
def seed_scoring_config(conn: sqlite3.Connection):
