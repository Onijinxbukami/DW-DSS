...
# CoreBank
# ---------------------------------------------------------------------------
def load_fact_corebank(conn: sqlite3.Connection):
    ...
    cid_to_cif = dict(zip(cb_cust["customer_id"], cb_cust["cif"]))
    ...
    # Aggregate payments: sum amount, take latest date per (loan_id, installment_no)
    ...
    merged = cb_sched.merge(
        pay_agg,
        on=["mortgage_loan_id", "installment_no"],
        how="left"
    )
    ...
    total = conn.execute(
        "SELECT COUNT(*) FROM fact_debt_installment WHERE product_source='COREBANK'"
    ).fetchone()[0]
...
# CoreCard
# ---------------------------------------------------------------------------
def load_fact_corecard(conn: sqlite3.Connection):
    ...
    # Aggregate payments per statement
    ...
    # Add installment_no (sequence per account, ordered by statement_date)
    ...
    # Merge statements with payments
    ...
    total = conn.execute(
        "SELECT COUNT(*) FROM fact_debt_installment WHERE product_source='CORECARD'"
    ).fetchone()[0]
