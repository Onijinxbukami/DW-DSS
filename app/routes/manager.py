...
def dashboard():
    ...
    kpi_overdue = conn.execute(
        "SELECT COUNT(DISTINCT contract_no) FROM dm_daily_collection_tasks "
        "WHERE dpd_current > 0 AND snapshot_date = ?", (SNAPSHOT_DATE,)
    ).fetchone()[0] or 0

    kpi_outstanding = conn.execute(
        "SELECT COALESCE(SUM(total_outstanding),0) FROM dm_daily_collection_tasks "
        "WHERE dpd_current > 0 AND snapshot_date = ?", (SNAPSHOT_DATE,)
    ).fetchone()[0] or 0

    total_tasks = conn.execute(
        "SELECT COUNT(*) FROM dm_daily_collection_tasks WHERE snapshot_date = ?",
        (SNAPSHOT_DATE,)
    ).fetchone()[0] or 1

    done_tasks = conn.execute(
        "SELECT COUNT(*) FROM dm_daily_collection_tasks "
        "WHERE task_status='DONE' AND snapshot_date = ?", (SNAPSHOT_DATE,)
    ).fetchone()[0] or 0

    kpi_done_pct = round(100.0 * done_tasks / total_tasks, 1) if total_tasks else 0

    kpi_active_collectors = conn.execute(
        "SELECT COUNT(DISTINCT collector_sk) FROM dm_daily_collection_tasks "
        "WHERE snapshot_date = ? AND collector_sk IS NOT NULL", (SNAPSHOT_DATE,)
    ).fetchone()[0] or 0

    # DPD bucket distribution
    bucket_rows = conn.execute(
        "SELECT dpd_bucket, COUNT(*) as cnt FROM dm_daily_collection_tasks "
        "WHERE snapshot_date = ? GROUP BY dpd_bucket ORDER BY dpd_bucket",
        (SNAPSHOT_DATE,)
    ).fetchall()
    ...
    channel_rows = conn.execute(
        ...
        "WHERE snapshot_date = ? GROUP BY assigned_channel",
        (SNAPSHOT_DATE,)
    ).fetchall()
